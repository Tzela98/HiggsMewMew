...
def main():
    model_path = "ML/projects/vbf_all_backgrounds/vbf_all_backgrounds_epoch70.pth"
    data_path = "/work/ehettwer/HiggsMewMew/ML/projects/vbf_all_backgrounds/vbf_all_backgrounds_test.csv"
    base_path = '/work/ehettwer/HiggsMewMew/ML/projects/vbf_all_backgrounds/'

    output_path = base_path + 'vbf_all_backgrounds_test_with_predictions.csv'
    ...
    df = pd.DataFrame({
        ...
        'true_labels': combined_data['signal'], 
    ...
    scale_signal = 40  # Scaling factor for signal
    n_sig, bins_sig = np.histogram(df_true_labels['predictions'], weights=df_true_labels['weights']*5*scale_signal, bins=10, range=(0, 1))
    err_sig = np.sqrt(np.histogram(df_true_labels['predictions'], weights=(df_true_labels['weights']*5)**2, bins=10, range=(0, 1))[0])
    ...
    plt.legend()
    plt.savefig(base_path + 'neural_network_output_vbf.png', bbox_inches='tight')
