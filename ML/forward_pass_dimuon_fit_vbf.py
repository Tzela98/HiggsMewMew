...
def main():
    base_path = '/work/ehettwer/HiggsMewMew/ML/projects/vbf_all_backgrounds_no_mH_run2/'
    model_path = "/work/ehettwer/HiggsMewMew/ML/projects/vbf_all_backgrounds_no_mH_run2/vbf_all_backgrounds_no_mH_run2_epoch40.pth"
    data_path = "/work/ehettwer/HiggsMewMew/ML/projects/vbf_all_backgrounds_no_mH_run2/vbf_all_backgrounds_no_mH_run2_test.csv"
    output_path = base_path + 'test_plus_prediction.csv'
    ...
    df = pd.DataFrame({
        ...
        'true_labels': filtered_df['signal'], 
        'weights': filtered_df['weights'],
        'm_H': filtered_df['m_vis']
    })
    ...
    eff_signal = np.sum(df_true_labels['weights']) / np.sum(combined_data[combined_data['signal'] == True]['weights'])
    eff_background = np.sum(df_false_labels['weights']) / np.sum(combined_data[combined_data['signal'] == False]['weights'])
    ...
    bin_errors = np.sqrt(np.histogram(df_true_labels['m_H'], weights=(df_true_labels['weights']*5)**2, bins=8, range=(115, 135))[0])
    ...
    plt.legend()
    plt.savefig(base_path + f'vbf_dimuon_mass_cut{int(cut*10):02}to1.png', bbox_inches='tight')
